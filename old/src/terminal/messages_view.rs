...
use crate::backend::{Game, MessageKind, Point, Size};
use std::io::Write;
...
impl MessagesView {
    pub fn render(&self, stdout: &mut Box<dyn Write>, game: &Game) {
        ...
        let bg = Color::White;
        for message in game.messages(self.size.height as usize) {
            let fg = to_fore_color(message.kind);
...
pub fn to_fore_color(kind: MessageKind) -> Color {
    use MessageKind::*;
    match kind {
        Critical => Color::Red,
        Error => Color::Red,
