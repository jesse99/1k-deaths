...
where
    ...
    C: Copy + Ord + Add + AddAssign + Default, // for movement C will normally be Time
...
mod tests {
    ...
    fn test_no_solution() {
        ...
        let callback = |loc: Point, neighbors: &mut Vec<(Point, i32)>| successors(&map, loc, neighbors);
    ...
    fn successors(map: &Vec2d<char>, loc: Point, neighbors: &mut Vec<(Point, i32)>) {
        ...
        for delta in deltas {
            ...
            if new_loc.x >= 0 && new_loc.y >= 0 && new_loc.x < size.width && new_loc.y < size.height {
                if *map.get(new_loc) == '~' {
                    neighbors.push((new_loc, 5));
                } else if *map.get(new_loc) != '#' {
                    if loc.diagnol(new_loc) {
                        neighbors.push((new_loc, 1));
                    } else {
                        neighbors.push((new_loc, 1));
                    }
                }
