use super::{to_termion, Color};
use crate::backend::{Point, Size};
use std::io::Write;
...
impl TextView {
    ...
    pub fn render(&self, stdout: &mut Box<dyn Write>) {
        ...
        let _ = write!(stdout, "{}", termion::color::Bg(to_termion(self.bg)));
    ...
    fn render_run(&self, stdout: &mut Box<dyn Write>, run: &TextRun, h: u16, v: u16) -> u16 {
        match run {
            ...
            TextRun::Color(color) => {
                let _ = write!(stdout, "{}", termion::color::Fg(to_termion(*color)));
            }
        }
        h
    }
