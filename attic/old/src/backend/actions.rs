//! Methods that actually do something. These are called as the result of a player action
//! (eg bump cell to the right) or as the result of NPC AI (move towards player).
use super::*;
...
impl Game {
    ...
    // /// A character was moved by an external force (e.g. a stronger character shoved it
    // /// away). This can happen even if the character doesn't have enough energy to do a
    // /// normal move and the character will take an energy hit.
    ...
    pub fn do_move(&mut self, oid: Oid, old_loc: Point, new_loc: Point) {
        ...
        self.level.move_char(oid, new_loc);
        if oid.value == 0 {
            let terrain = self.level.get_terrain(new_loc);
            self.player_entered(terrain);

            self.level.pov.dirty();
        }
    }

    pub fn do_open_door(&mut self, oid: Oid, ch_loc: Point, obj_loc: Point) {
        debug!("{oid} is opening the door at {obj_loc}");
        self.level.set_terrain(obj_loc, Terrain::OpenDoor);
