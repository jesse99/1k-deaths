...
use fnv::FnvHashMap;
use one_thousand_deaths::{Color, State};
use termion::event::Key;

type KeyHandler = fn(&mut TextMode, &mut State) -> InputAction;
...
impl TextMode {
    fn create(lines: Vec<Line>, at_top: bool, bg: Color) -> TextMode {
        ...
        // commands.insert(Key::PageDown, Box::new(|s, state| s.do_page(state, 1))); // TODO: we're not catching these
        commands.insert(Key::Char(' '), Box::new(|s, state| s.do_page(state, 1)));
        commands.insert(Key::Char('f'), Box::new(|s, state| s.do_page(state, 1)));
        commands.insert(Key::Ctrl('f'), Box::new(|s, state| s.do_page(state, 1)));
        commands.insert(Key::Ctrl('v'), Box::new(|s, state| s.do_page(state, 1)));

        // commands.insert(Key::PageUp, Box::new(|s, state| s.do_page(state, -1)));
        commands.insert(Key::Char('b'), Box::new(|s, state| s.do_page(state, -1)));
        commands.insert(Key::Ctrl('b'), Box::new(|s, state| s.do_page(state, -1)));

        commands.insert(Key::Down, Box::new(|s, state| s.do_scroll(state, 1)));
        commands.insert(Key::Char('\n'), Box::new(|s, state| s.do_scroll(state, 1)));
        commands.insert(Key::Char('e'), Box::new(|s, state| s.do_scroll(state, 1)));
        commands.insert(Key::Char('j'), Box::new(|s, state| s.do_scroll(state, 1)));
        commands.insert(Key::Ctrl('e'), Box::new(|s, state| s.do_scroll(state, 1)));
        commands.insert(Key::Ctrl('j'), Box::new(|s, state| s.do_scroll(state, 1)));
        commands.insert(Key::Ctrl('n'), Box::new(|s, state| s.do_scroll(state, 1)));

        commands.insert(Key::Up, Box::new(|s, state| s.do_scroll(state, -1)));
        commands.insert(Key::Char('k'), Box::new(|s, state| s.do_scroll(state, -1)));
        commands.insert(Key::Char('p'), Box::new(|s, state| s.do_scroll(state, -1)));
        commands.insert(Key::Char('y'), Box::new(|s, state| s.do_scroll(state, -1)));
        commands.insert(Key::Ctrl('k'), Box::new(|s, state| s.do_scroll(state, -1)));
        commands.insert(Key::Ctrl('p'), Box::new(|s, state| s.do_scroll(state, -1)));
        commands.insert(Key::Ctrl('y'), Box::new(|s, state| s.do_scroll(state, -1)));

        commands.insert(Key::Char('d'), Box::new(|s, state| s.do_scroll_by(state, 1)));
        commands.insert(Key::Ctrl('d'), Box::new(|s, state| s.do_scroll_by(state, 1)));

        commands.insert(Key::Char('u'), Box::new(|s, state| s.do_scroll_by(state, -1)));
        commands.insert(Key::Ctrl('u'), Box::new(|s, state| s.do_scroll_by(state, -1)));

        // commands.insert(Key::Home, Box::new(|s, state| s.do_scroll_to_start(state)));
        // commands.insert(Key::End, Box::new(|s, state| s.do_scroll_to_end(state)));
        commands.insert(Key::Char('?'), Box::new(|s, state| s.do_help(state)));
        commands.insert(Key::Char('q'), Box::new(|s, state| s.do_pop(state)));
        commands.insert(Key::Esc, Box::new(|s, state| s.do_pop(state)));
...
[[escape]] and [[q]] exit this mode."#;
    ...
    fn do_page(&mut self, _game: &mut State, sign: i32) -> InputAction {
        self.text.scroll(sign * self.text.size().height);
        InputAction::UpdatedGame
    }

    fn do_pop(&mut self, _game: &mut State) -> InputAction {
        InputAction::Pop
    }

    fn do_scroll(&mut self, _game: &mut State, delta: i32) -> InputAction {
        self.text.scroll(delta);
        InputAction::UpdatedGame
    }

    fn do_scroll_by(&mut self, _game: &mut State, sign: i32) -> InputAction {
