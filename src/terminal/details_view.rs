use super::color;
use one_thousand_deaths::render;
use one_thousand_deaths::{Color, Point, Size, State};
use std::io::Write;
...
impl DetailsView {
    pub fn render(&self, stdout: &mut Box<dyn Write>, state: &State) {
        ...
        self.render_player(h, &mut v, stdout, state);
        // self.render_npcs(h, &mut v, stdout, state);
        self.render_trailer(h, v, stdout);
    }

    fn render_player(&self, h: u16, v: &mut u16, stdout: &mut Box<dyn Write>, state: &State) {
        let (current, max) = render::player_hps(state);
    ...
    // fn render_npcs(&self, h: u16, v: &mut u16, stdout: &mut Box<dyn Write>, state: &State) {
    //     let npcs = state.npcs(super::wizard_mode());
