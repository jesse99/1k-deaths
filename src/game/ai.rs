...
/// Called for objects that may want to perform an action after some minimum amount of
/// time has passed. Returns Some((duration, extra)) if the object decided to perform an
/// action. extra is additional time subtracted from the object's time units but not added
